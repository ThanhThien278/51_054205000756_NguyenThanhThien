...
void QuanLyPhongHoc::xoaPhongHoc(const string& maPhong) {
    auto it = remove_if(danhSachPhongHoc.begin(), danhSachPhongHoc.end(), [&maPhong](Phong* phong) {
        return phong->getMaPhong() == maPhong;
        });

    if (it != danhSachPhongHoc.end()) {
        delete* it;
        danhSachPhongHoc.erase(it, danhSachPhongHoc.end());
        cout << "Phong hoc da duoc xoa." << endl;
    }
    else {
